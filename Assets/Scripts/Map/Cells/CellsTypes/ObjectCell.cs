...
public class ObjectCell : CellTypeBase, ISpotType
{
    public Cell3D m_Cell;
    public GameObject ObjSpotPref;
    [HideInInspector]
    public GameObject m_ObjSpot;
    ...
    private void OnEnable()
    {
        if (m_ObjSpot == null)
        {
            ...
            {
                m_ObjSpot = GetComponentInChildren<KeySpot>().gameObject;
            }
            else
            {
                m_ObjSpot = Instantiate(ObjSpotPref, transform);
            }
        }
        SpotSetup(m_ObjSpot, ObjSpotPref);
    }
    private void Update()
    {
        UpdateSpotData(m_ObjSpot);
    }
